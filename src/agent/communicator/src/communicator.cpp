...
namespace communicator
{
    ...
    boost::asio::awaitable<void> Communicator::GetCommandsFromManager(std::function<void(const std::string&)> onSuccess)
    {
        ...
        co_await m_httpClient->Co_PerformHttpRequest(
            m_token, reqParams, {}, onAuthenticationFailed, m_retryInterval, m_batchingInterval, onSuccess, loopCondition);
    }
                                                ...
                                                std::function<void(const std::string&)> onSuccess)
    {
        ...
        co_await m_httpClient->Co_PerformHttpRequest(
            m_token, reqParams, getMessages, onAuthenticationFailed, m_retryInterval, m_batchingInterval, onSuccess, loopCondition);
    }
